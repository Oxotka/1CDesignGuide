...
- В случае есть текст из одного предложения, то в конце точка не ставится
...
<img src=img/checkboxTitle.png width=300 border="1">
...
2. Зона действия, с точки зрения интерфейса, у флажка небольшая или отсутствует. Например, при включении флажка можно разблокировать соседнюю гиперссылку. В случае если нужно влиять на форму сильнее, то лучше рассмотреть [Переключатель](#переключатель)
...
4. Следует с осторожностью использовать флажки для негативных опций. Чтобы его понять потребуется дополнительное усилие, так как нужно следить и за включенностью/выключенностью флажка и за самим текстом с "Не"
...
3. Не всегда нужно использовать заголовок поля ввода. Например, в шапке отчета мы не пишем у поля Организация заголовок, так как пользователи знают свою организацию и понять что в этом поле указано по значению
