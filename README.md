...
1. Декорация часто используется как подсказка на форме. В этом случае часто используют Форматированную строку, чтобы добавить гиперссылки и оформление
2. Если текст из одного предложения, то в конце точка не ставится
3. Не следует использовать декорацию если она, не выполняет полезное действие
4. Не следует использовать для установки заголовки простых полей. В этом случае не срабатывает автофокус на поле и часто забывают поставить двоеточие в заголовке поля
5. Не следует использовать для выравнивания элементов. Лучше использовать для этого свойства [Групп](#группы)
...
1. Картинка может использовать для иллюстрации или чтобы привлечь внимание пользователя
2. Картинка может быть в составе форматированной строки или отдельным элементом
3. Не следует использовать картинку из файла, ее следует добавить в библиотеку картинок
...
1. Ширина поля должна быть такой, чтобы вмешать самое длинное значение из списка
2. Если в поле ввода предусмотрено удаление или пустое значение, то нужно добавить кнопку Очистки (x). Кроме, отборов на формах, там используется флажок
3. Для строковых полей ввода иногда следует предусмотреть кнопку выбора, например, выбор Пути к файлу или например выбор Наименования
4. Для строковых полей более 100 символов следует рассмотреть использование многострочного поля. Для многострочного поля заголовок обычно устанавливается сверху
5. Для реквизитов формы следует предусмотреть сохранение значений пользователя, например, поля с периодом или Организация
6. Не всегда нужно использовать заголовок поля ввода. Например, в шапке отчета мы не пишем у поля Организация заголовок, так как пользователи знают свою организацию и понять что в этом поле указано по значению

    <details><summary>Как выглядит В ЕДТ</summary>
     <img src=img/organization.png width=700>
    </details>

7. В случае если заголовок не указываем, то обязательно должна быть подсказка ввода. То есть для Организаций в отчете следует писать: По всем организациям. Таким образом, показываем, что сейчас символизирует собой этой поле
8. Не следует использовать платформенный выбор типа для составных типов. Если используется разный тип, то следует добавить отдельный выбор типа, например, Контрагент или Физическое лицо
...
4. Не следует использовать кнопку для перехода к другой форме, для этого используйте [Гиперссылку](#гиперссылки)
5. В случае если по кнопке будет открываться новая форма, то название кнопки должно сообщать об этом. Например, "Показать настройки"
