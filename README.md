...
3. Не используйте декорацию для установки заголовка простых полей. В этом случае не срабатывает автофокус на поле и часто забывают поставить двоеточие в заголовке поля
...
5. Используйте многострочное поле ввода для строковых значений длиннее 100 символов. Установите заголовок для многострочного поля сверху
...
3. Добавьте подсказки к значениям переключателя для сложных опций
    ...
    Для этого создайте два элемента формы с путем к одному реквизиту, у каждого элемента можно сделать свою подсказку
...
2. Опишите утвердительное действие, чтобы пользователь однозначно понял, что будет выполнено. Для этого установите текст этой кнопки, чтобы он повторял запрашиваемое действие
...
2. Используйте подсказку сверху о всей форме, если она используется не часто. У пользователя такая подсказка будет всегда перед глазами. Например, если это форма с настройками правил
...
3. Уберите подсказки в группе созданные платформой по умолчанию - они выводятся при удержании курсора над группой
...
1. Используйте свертываемую группу, чтобы скрыть элементы - это делает форму компактнее и удобнее для работы.
...
4. Используйте для программного управления свойства группы Свернута методы Группа.Показать() и Группа.Скрыть()

5. Не помещайте в свертываемую группу элементы, которые требуются часто или являются обязательными

6. Не размещайте свертываемую группу выше обычных групп
